...
- 被工具层标记为 `is_close(平仓或减仓)` 的意图会豁免新增风险限制；限价减仓仍受委托价偏离规则约束，且官方合约规格临时不可用时仍可使用持仓快照降险。
...
`place_order(下单)` 对新增敞口接收 `margin_usdt(本单投入保证金)`、`leverage(杠杆)`、`side(方向)` 和必填止损，固定使用逐仓；代码按市价单标记价或限价单委托价、实时 `quanto_multiplier(每张对应币数)` 与数量上下限换算 Gate 张数，并回算实际保证金、名义价值、预计手续费和整仓计划止损金额。LLM 工具协议不暴露 `size(张数)`。整仓平仓使用 `close=true`，部分减仓使用 `reduce_pct(减仓比例)`，反手必须先平仓再另开反向仓位。

live/testnet 新增敞口逐次读取对应 Gate 环境的合约规格；paper 在新增敞口前通过正式网公共接口刷新规格并更新内存。查询失败、合约不可交易或正在下架时拒绝新增敞口，不使用本地规格文件或定时刷新任务兜底。`amend_order(改单)`只允许改价，改金额必须撤单后重新下单。
...
| `risk.max_position_pct(单仓名义价值上限)` | 同一合约已有持仓、未成交挂单和本单合计名义价值相对权益的上限 | 校验 | 豁免 |
| `risk.max_total_position_pct(总持仓权益占比上限)` | 当前持仓加本单后的总名义价值上限 | 校验 | 豁免 |
| `risk.max_position_stop_risk_pct(单仓计划止损风险上限)` | 单个合约整仓从开仓均价到止损价的理论亏损相对权益上限，默认 1%；盈利区止损按零亏损 | 校验 | 豁免 |
...
计划止损金额只计算开仓均价到止损触发价的理论价格损失，不计手续费、滑点、跳空或强平距离。加仓按预计新均价重算整个合约持仓；修改止损也按真实持仓重新校验。当前不设置 BTC、ETH 等不同合约之间的组合止损风险上限，组合层仍由总名义价值、杠杆和日亏损等规则约束。
