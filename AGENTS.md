...
1. **分支**：禁止直推 main（GitHub 分支保护强制 PR + CI 全绿）。用户授权实施代码、规则或文档变更后，必须在修改前从最新 main 创建对应的 `feat/xxx`、`fix/xxx` 或 `chore/xxx` 专用分支；若当前已在同一任务的专用分支则继续使用，不得为同一功能的后续对话重复建分支。一个分支只做一件事，1–3 天内合回。诊断、评审、方案讨论等只读任务不建分支；用户明确要求不建分支时服从用户要求
...
3. **提交时机**：开发过程中每达到一个稳定逻辑断点就主动创建本地提交，不等整个功能全部完成后再集中拆分。稳定逻辑断点必须同时满足：该阶段行为完整且不是不可运行的半成品；对应测试已通过；暂存区只包含本逻辑切片；提交可独立理解、审查和回滚。小步提交按业务行为拆分而非按单文件或零散代码行拆分；测试失败、接口只改一半或跨层契约不一致时不得作为稳定断点提交。用户明确要求不提交时不得提交
4. **默认开 PR**：**PR 是功能单位——一个 PR = 一个完整功能改动**。任务完成并通过规定审查、测试和门禁后，默认直接推送专用分支并创建 PR，不必等待用户再次要求；只有用户明确要求不推送、不建 PR 或仅保留本地修改时才停止在本地。默认 PR 必须说明改动范围、验证证据和兼容判断
5. **合并**：PR 创建后等待 CI 三 job（backend/frontend/e2e）全绿并给出改动摘要；最终合并必须由**人手动确认**，AI 协作者不得擅自合并。采用 squash merge，合并即删分支
6. **提交归属**：AI 协作者参与的提交以协作者账号（djfch）直接提交、不额外添加 Co-authored-by 等协作标记——此为既定政策，视为正常署名而非隐匿；人工提交使用个人账号
7. **生产基线**：`main(主分支)`是唯一部署来源；`deploy(生产基线分支)`只指向最后一次通过服务器健康检查的完整提交，禁止作为开发基线或 PR 目标，也禁止删除和强推
8. **大改动**（≥3 个文件或 >100 行）必须执行以下流程：
   1. 修复问题时使用第一性原理：先定义用户可观察行为；再找出系统不可违反的不变量；然后把不变量转成可验证规则（单元测试、集成测试、类型约束、数据库约束、断言、CI 检查）；不接受只修表面现象的补丁
   2. 实现完成后，派**一个审查 subagent** 做审查：主动寻找 bug、遗漏边界、架构风险、测试缺口。**审查 subagent 一律显式绑定主模型（派单时指定 primary），不走 secondary 便宜模型——审查找的是自己看不见的错，省钱即降质**。任务说明要求分遍审查（第 1 遍只看跨层/前后端契约一致性，第 2 遍只看边界与失败态，第 3 遍只看测试缺口），不按检查项拆多个审查 agent
   3. 审查采用**循环修复模式**：审查 agent 报告问题 → 主 agent 对真实且重要的问题先补回归测试、再按第一性原理做最小正确修复 → 再次派审查 agent 复审；如此往复，直到审查 agent 明确给出"可以合并"的结论，才可推送 GitHub 并开 PR。审查 agent 未放行前，禁止推送、禁止开 PR。评审前已按稳定逻辑断点创建的本地提交可以保留，评审修复另建提交
