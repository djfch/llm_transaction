...
覆盖：
- GET /api/research/reports(+{id})：分页/最新在前/narrative 截断 200/含失败记录；
  详情全文、404、evidence/risks/raw 解析为对象（非法 JSON 保留原文）、causal_links 解析与空数组；
...
async def test_reports_list_pagination_and_truncation(repo: Repo, tmp_path: Path):
    """列表：分页、最新在前、narrative 截断 200、失败记录也在列。"""
    r1 = await repo.research.save_report(
        report_type="asia", direction="偏多", confidence="高", narrative="短叙述"
    )
    r2 = await repo.research.save_report(
        report_type="manual",
        direction="偏空",
        confidence="中",
    ...
    r3 = await repo.research.save_report(
        report_type="us", direction="中性", confidence="低", error="LLM 超时"
    )
    async with _client_of(_deps(repo, tmp_path)) as c:
        body = (await c.get("/api/research/reports")).json()
        assert body["total"] == 3
        assert [i["id"] for i in body["items"]] == [r3.id, r2.id, r1.id]  # 最新在前
        item = body["items"][1]
        assert set(item) == {  # 列表项为 model_dump 全量键（evidence_json 等原名不动）
            "id",
            "report_type",
            "direction",
            "confidence",
        ...
        assert len(item["narrative"]) == 200  # 列表截断
        assert item["round_id"] == "rs-round-1"
        assert body["items"][0]["error"] == "LLM 超时"  # 失败记录也在列
        ...
        assert far["items"] == [] and far["total"] == 3  # 越界页 items 空但 total 准


# ---------- GET /api/research/reports/{id} ----------


async def test_report_detail_full_and_json_fields(repo: Repo, tmp_path: Path):
    """详情：narrative 全文；evidence/risks/raw 解析为对象，非法 JSON 保留原文；404。"""
    report = await repo.research.save_report(
        report_type="manual",
        direction="偏多",
        confidence="高",
        narrative=_LONG_NARRATIVE,
    ...
    bad = await repo.research.save_report(
        report_type="asia",
        direction="中性",
        confidence="低",
    ...
    async with _client_of(_deps(repo, tmp_path)) as c:
        ...
        assert detail["narrative"] == _LONG_NARRATIVE  # 详情全文
        assert detail["evidence"] == [{"timeline_id": 1, "note": "ETF 净流入"}]
        assert detail["risks"] == ["宏观数据反复"]
        assert detail["raw"] == {"llm": "原文"}
        assert "evidence_json" not in detail  # *_json 原名换成解析对象键
        assert detail["causal_links"] == []  # 无因果链 → 空数组
        bad_detail = (await c.get(f"/api/research/reports/{bad.id}")).json()
        assert bad_detail["evidence"] == "非JSON原文"  # 非法 JSON 保留原文
        assert bad_detail["risks"] == "[断裂"
...
async def test_report_detail_causal_links(repo: Repo, tmp_path: Path):
    ...
    r1 = await repo.research.save_report(report_type="manual", direction="偏多", confidence="高")
    r2 = await repo.research.save_report(report_type="asia", direction="偏空", confidence="中")
