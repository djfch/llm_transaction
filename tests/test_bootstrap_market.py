...
def _ticker(price: Decimal) -> Ticker:
    """构造指定价格的 BTC 永续 Ticker 测试数据（其余字段取固定占位值）。

    参数：
        price: Decimal，最新成交价
    返回：
        Ticker，返回该测试辅助函数构造或记录的结果
    """
    return Ticker(
        contract=BTC,
        last=price,
...
async def build_ctx(tmp_path: Path) -> AsyncIterator[Callable[..., AppContext]]:
    ...
    返回：
        AsyncIterator[Callable[..., AppContext]]，返回该测试辅助函数构造或记录的结果
    """
