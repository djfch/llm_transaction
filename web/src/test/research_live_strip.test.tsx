 ...
 * 不依赖 WS 激活、面板 409 catchup 事件（research-round-catchup）补漏激活、WS 断线恢复后重跑补漏、
 * 3 秒轮询刷新工具链、轮次绑定（点火后先返回上一轮已结束记录保持等待不误退、见到进行中轮绑定 round_id）、
 * 代际校验（上一激活周期的迟到 /live 响应不关闭新一轮）、点火兜底（约 90 秒不见进行中轮视为点火失败退出）、
 * 双通道结束（WS research_round / 轮询确认本轮 ended_at 非空）停止轮询并回调 onFinished、
...
describe('ResearchLiveStrip(研报进行中进度条)', () => {
  ...
  it('点火后 /live 先返回上一轮已结束记录 → 保持等待不退出；随后返回进行中轮 → 绑定并展示工具链', async () => {
    const onFinished = vi.fn()
    await renderStrip(onFinished)
    // 上一轮的历史记录（rs-old，起点火之前就已结束）：新后台任务还没 begin_round 时 /live 返回的就是它
  ...
  it('面板 409 catchup 事件 research-round-catchup → 补漏发现进行中轮并激活（WS 断线场景）', async () => {
