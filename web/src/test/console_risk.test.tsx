...
describe('RiskPanel(硬性风控 · 代码保证)', () => {
  it('渲染四行风控参数（比例 ×100 百分比）+ 裁决说明', async () => {
    ...
    expect(await screen.findByText('单仓占比上限')).toBeInTheDocument()
    expect(screen.getByText('30%')).toBeInTheDocument()
    expect(screen.getByText('总仓占比上限')).toBeInTheDocument()
    expect(screen.getByText('80%')).toBeInTheDocument()
    expect(screen.getByText('最大杠杆倍数')).toBeInTheDocument()
    expect(screen.getByText('5x')).toBeInTheDocument()
    expect(screen.getByText('单日最大下单数')).toBeInTheDocument()
    expect(screen.getByText('20')).toBeInTheDocument()
    expect(screen.getByText('最大价格偏离')).toBeInTheDocument()
    expect(screen.getByText('2%')).toBeInTheDocument()
    expect(screen.getByText('单日亏损锁仓阈值')).toBeInTheDocument()
    expect(screen.queryByText(/max_position_pct|max_total_position_pct|max_deviation|daily_loss_limit/)).not.toBeInTheDocument()
