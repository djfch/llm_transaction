...
export default function ResearchPanel() {
   ...
   *  点火成功广播 research-round-ignite（detail.roundId 为后端预分配的审计轮 ID，与 WS 轮始事件同一标识）：
   *  状态条不依赖 WS 也能激活并按 ID 绑定本轮（覆盖 WS 断线窗口内点火）。
   *  409（生成中）按成功样式提示而非错误红，并广播 research-round-catchup：任务由他处（别的标签页/自动调度）
   *  点火而本页 WS 断线时，状态条以轮询发现模式找回进行中轮；503/422 经 ApiError.detail 红字提示。 */
  const runNow = async () => {
    setRunning(true)
    setNotice(null)
    try {
      const res = await api.runResearch('manual', 24)
      setNotice({ ok: true, text: '研报已启动，进度见下方状态条' })
      window.dispatchEvent(new CustomEvent('research-round-ignite', { detail: { roundId: res.roundId } }))
    } catch (e) {
