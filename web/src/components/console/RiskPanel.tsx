/**
 * 硬性风控速览（方案 C 左下面板）：标题「硬性风控 · 代码保证」+ 六行风控参数 + 裁决说明。
        ...
        {risk && (
          <>
            <ul className="space-y-2 text-[11px]">
              <RiskRow label="单仓占比上限" value={fmtPct(risk.max_position_pct)} />
              <RiskRow
                label="总仓占比上限"
                value={fmtPct(risk.max_total_position_pct)}
              />
              <RiskRow label="最大杠杆倍数" value={`${risk.max_leverage}x`} />
              <RiskRow label="单日最大下单数" value={String(risk.max_orders_per_day)} />
              <RiskRow label="最大价格偏离" value={fmtPct(risk.max_deviation)} />
              <RiskRow
                label="单日亏损锁仓阈值"
