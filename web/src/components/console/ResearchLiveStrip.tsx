/**
 * 研报进行中进度条：研报面板顶部的实时状态条，按轮次 ID 认轮（不比较任何时间）。
 * 进入进行中态三条通路：
 ...
 * 退出幂等：清空绑定/可信标记/seen/计时，停轮询并回调 onFinished（父组件据此刷新研报列表）。
 * 每次进入进行中态代际（generation）+1，轮询响应按发送时的代际校验，上一激活周期的迟到响应
 * 直接丢弃；轮询失败静默保留进度条。WS research_round 事件仅 round_id 与绑定相符（或未绑定）
...
function isZombieRound(round: ResearchLiveRound, nowMs: number): boolean {
...
export default function ResearchLiveStrip({ onFinished }: ResearchLiveStripProps) {
  ...
  /** 退出进行中态（幂等）：清空轮次绑定/可信标记/seen/计时；停止轮询由 active effect 清理联动；notify 时通知父组件研报已结束 */
  ...
  // WS：研报轮开始 → pinned 进入（绑定其 round_id）；轮结束 → 仅 round_id 与绑定相符（或未绑定）才退出，不符忽略
  useEffect(() => {
    if (!lastMessage) return
    if (lastMessage.type === 'research_round_start') enterActive(lastMessage.data.round_id, true)
  ...
  useEffect(() => {
    window.addEventListener('research-round-catchup', onCatchup)
    return () => window.removeEventListener('research-round-catchup', onCatchup)
  }, [onCatchup])
