 ...
 * 进入进行中态（三条通路）：WS 收到 research_round_start（直接绑定其 round_id）；面板点火事件
 * research-round-ignite（WS 断线窗口内手动点火的兜底，不依赖 WS；先记点火时刻，待 /live 首次返回
 * 进行中轮时绑定其 round_id）；挂载 / WS 重连 / 面板 409 catchup 事件（research-round-catchup）补漏
 * 发现进行中的研报轮（绑定该轮 round_id；started_at 距今 ≤30 分钟，超出的视为僵尸轮不展示）。
 * 进行中每 3 秒轮询 /api/research/live 刷新工具链；每次进入进行中态代际（generation）+1，
 * 轮询响应按发送时的代际校验，上一激活周期的迟到响应直接丢弃。
 * 退出进行中态：WS 收到 research_round（事件不重放，收到的必为当期，收到即退出），或轮询确认
 ...
 * 与点火时刻、停止轮询并回调 onFinished（父组件据此刷新研报列表）。轮询失败静默保留进度条。
 */
...
function isOwnEndedRound(round: ResearchLiveRound, boundId: string | null, ignitedAtMs: number | null): boolean {
...
export default function ResearchLiveStrip({ onFinished }: ResearchLiveStripProps) {
  ...
  /** 退出进行中态（幂等）：清空轮次绑定与点火时刻；停止轮询由 active effect 清理联动；notify 时通知父组件研报已结束 */
  ...
  /** 拉一次实时研报状态：进行中轮刷新工具链（未绑定则绑定）；确认本轮结束才退出（旧轮历史记录保持等待，点火路径有兜底期限）；迟到响应按代际丢弃；失败静默 */
  ...
  // WS：研报轮开始 → 进入进行中态（绑定其 round_id）；轮结束 → 退出并通知（事件不重放，收到的必为当期）
  useEffect(() => {
    if (!lastMessage) return
    if (lastMessage.type === 'research_round_start') enterActive(lastMessage.data.round_id)
    else if (lastMessage.type === 'research_round') exitActive(true)
  }, [lastMessage, enterActive, exitActive])

  /** 面板点火事件回调：WS 断线窗口内手动点火时不依赖 WS 的激活通路（与 research_round_start 同语义，但轮次待 /live 绑定） */
  const onIgnite = useCallback(() => enterActive(null), [enterActive])
  useEffect(() => {
    window.addEventListener('research-round-ignite', onIgnite)
    return () => window.removeEventListener('research-round-ignite', onIgnite)
  }, [onIgnite])
  ...
  /** 补漏查询：进行中且非僵尸轮则进入进行中态并绑定其 round_id（保留轮到的工具链）；查询失败静默视为无进行中研报 */
  ...
  useEffect(() => {
    window.addEventListener('research-round-catchup', catchUp)
    return () => window.removeEventListener('research-round-catchup', catchUp)
  }, [catchUp])
