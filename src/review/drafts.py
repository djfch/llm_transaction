...
def _channels(deps: ReviewToolDeps) -> list[tuple[Any, list[int], str]]:
    """组装本轮生效通道列表：（store、草稿 id 列表、通道中文名）。
    ...
    返回：
        list[tuple[Any, list[int], str]]：待处理通道列表
    """
    channels: list[tuple[Any, list[int], str]] = [
        (deps.store, deps.strategy_draft_ids, "策略"),
    ]
    if deps.indicator_config_store is not None:
        channels.append((deps.indicator_config_store, deps.indicator_draft_ids, "指标配置"))
...
async def apply_drafts(deps: ReviewToolDeps) -> None:
    ...
    """
    for store, draft_ids, label in _channels(deps):
