...
- run_now：手动触发同一昨日区间；与每日触发共用同一把锁，进行中返回忙
  （server 层映 409）；
...
class ReviewScheduler:
    ...
    async def _tick(self, now: float | None = None) -> None:
        ...
        if now < daily_fire_ts(self._settings.review.daily_time, now):
            return  # 未到当日触发时刻
        latest = await self._repo.latest_review_period_end()
    ...
    async def run_now(self) -> dict:
        """手动触发昨日区间复盘；进行中返回忙（server 层映 409）。"""
        if self._lock.locked():
            return {"started": False, "error": "复盘进行中"}
        day_start = local_day_start(time.time())
        async with self._lock:
            result = await self._agent.run(day_start - _SECONDS_PER_DAY, day_start)
