...
SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "submit_research_prompt_revision": {
        "description": (
            ...
            "（全文重写）。调用前必须先用无参 get_research_prompt_versions 读取当前"
            "提示词全文，未读取或只读历史版本的提交会被拒绝。服务端校验（≥100 字符、"
            "≤32KB、与当前版本有差异），通过则生成草稿版本 vN，本轮复盘报告提交成功后"
            "统一生效；拒绝则返回原因列表，修正后可重试。"
    ...
    "submit_indicator_config": {
        "description": (
            ...
            f"（去重后 1~8 个；可选键：{_INDICATOR_MENU}）。调用前必须先用 "
            "get_indicator_config 读取当前完整配置，未读取的提交会被拒绝。服务端校验"
            "（未知键/数量越界/与当前无差异/reason 为空），通过则生成新版本 vN 并于"
            "下一轮决策生效；拒绝则返回原因列表，修正后可重试。无实质复盘依据不要调用"
        ),
        "parameters": {
            "type": "object",
            "properties": {
