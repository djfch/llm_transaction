...
class ResearchPromptStore:
    ...
    async def apply_version(self, version_id: int) -> ResearchPromptVersion | None:
        ...
        返回：
            ResearchPromptVersion | None：已生效（applied）的版本对象；目标版本
    ...
    async def _apply_version_locked(self, version_id: int) -> ResearchPromptVersion | None:
        ...
        返回：
            ResearchPromptVersion | None：已生效版本；目标版本已是 applied 且最新
        ...
        # 避免被「实读基线已不在位」误反标 discarded（本通道 rollback 校验
        # status=='applied'，反标会让该版本永久不可回滚）
        if version.status == "applied" and latest is not None and latest.id != version.id:
            logger.debug(
                "研报提示词版本 v%d 已生效过，重放幂等返回（当前生效 v%d）", version_id, latest.id
            )
            return version
